...
public class PathPartitioner {
	...
	public Set<String> getPartitionKeys(String location, Configuration conf)
		...
		// this line ensures that if an absolute path is presented, that the
		// absolute path's partition keys are also included.
		String[] slashSplits = location.split("/");
		for (String slashSplit : slashSplits) {
			String[] keyValuePair = slashSplit.split("=");
			if (keyValuePair.length == 2) {
				
				partitionKeys.add(parseKey(keyValuePair[0]));
			}
		}
		System.out.println("Pass one: keys: " + Arrays.toString(partitionKeys.toArray()));

		// recurse through the directories to find all the partition keys
		parseAndPutKeyValue(location, partitionKeys);
		System.out.println("Pass two: keys: " + Arrays.toString(partitionKeys.toArray()));
		...
		System.out.println("Pass three: keys: " + Arrays.toString(partitionKeys.toArray()));
	...
	private final String parseKey(String key){
		...
		if(keyName.startsWith("/")){
			keyName = keyName.substring(1, keyName.length());
		}
	...
	private final void parseAndPutKeyValue(String pathName,
		...
		if (keyValue != null) {
			partitionKeys.put(parseKey(keyValue[0]), keyValue[1]);
		}
	...
	private final void parseAndPutKeyValue(String pathName,
		...
		String[] keyValue = parsePathKeyValue(pathName);
		if (keyValue != null) {
			partitionKeys.add(parseKey(keyValue[0]));
		}
